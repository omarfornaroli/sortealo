...
import { ShieldCheck, Ticket, User, Clock, ChevronLeft, CreditCard, Loader2, CheckCircle2, FlaskConical, Fingerprint, Store } from 'lucide-react';
...
function RaffleContent({ id }: { id: string }) {
  ...
  const sellerCode = searchParams.get('ref'); // Capturar vendedor del link
  ...
  const handleTestPurchase = async () => {
    ...
    try {
      const res = await fetch(`/api/raffles/${id}/participate`, {
        ...
        body: JSON.stringify({ ...formData, quantity, sellerCode }), // Enviamos sellerCode si existe
      });
    ...
    try {
      const res = await fetch('/api/mercadopago/preference', {
        ...
        body: JSON.stringify({
          ...
          quantity: 1,
          user: { ...formData, sellerCode } // Pasamos info del vendedor a MP metadata
        }),
      });
              ...
              <div>
                ...
                <div className="grid sm:grid-cols-2 gap-6 mt-8">
                  <div className="bg-slate-50 p-8 rounded-[2rem] border border-slate-100">
                    <span className="block text-[10px] uppercase text-slate-400 font-black mb-1 tracking-widest">Sorteo</span>
                    <span className="font-headline font-bold text-xl">{formattedDrawDate || 'Próximamente'}</span>
                  </div>
