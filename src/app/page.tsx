...
export default async function HomePage() {
  try {
    ...
    const serializedSettings = siteSettings ? JSON.parse(JSON.stringify(siteSettings)) : {};
    ...
    return (
      <div className="min-h-screen flex flex-col bg-white">
        ...
        <Sponsors 
          sponsors={serializedSettings.sponsors} 
          title={serializedSettings.sponsorsTitle} 
        />
          ...
          <div className="container mx-auto px-4">
            <div className="text-center mb-20 space-y-6">
              <div className="inline-block px-6 py-2 bg-primary/10 rounded-full text-primary font-black text-xs uppercase tracking-widest border border-primary/20">
                {serializedSettings.siteName || 'Sortealo'}
              </div>
              <h2 className="text-5xl lg:text-7xl font-headline font-bold text-slate-900 leading-none">
                {serializedSettings.activeRafflesTitle || 'Sorteos Activos'}
              </h2>
              <p className="text-slate-500 text-xl max-w-2xl mx-auto font-medium">
                {serializedSettings.activeRafflesSubtitle || 'Seleccioná tu premio, elegí tus chances y participá.'}
              </p>
            </div>
