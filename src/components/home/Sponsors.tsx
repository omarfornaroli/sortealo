  ...
  return (
    <section className="py-16 bg-white border-y border-slate-100 overflow-hidden">
      <div className="container mx-auto px-4 mb-10">
        <p className="text-center text-[12px] font-black text-slate-400 uppercase tracking-[0.4em]">
          Nuestros Sponsors
        </p>
      </div>
      ...
      <div className="relative flex overflow-x-hidden">
        ...
        <div className="flex animate-marquee whitespace-nowrap items-center py-4">
          {marqueeItems.map((url, index) => {
            if (!url) return null;
            return (
              <div key={index} className="mx-12 md:mx-20 flex items-center justify-center shrink-0">
                <div className="relative h-[72px] w-[192px] md:h-[84px] md:w-[264px] group grayscale hover:grayscale-0 transition-all duration-500 opacity-60 hover:opacity-100">
                  <Image 
                    src={url} 
                    alt={`Sponsor ${index}`} 
                    fill 
        ...
        <div className="absolute top-0 flex animate-marquee2 whitespace-nowrap items-center py-4 h-full">
          {marqueeItems.map((url, index) => {
            if (!url) return null;
            return (
              <div key={`dup-${index}`} className="mx-12 md:mx-20 flex items-center justify-center shrink-0">
                <div className="relative h-[72px] w-[192px] md:h-[84px] md:w-[264px] group grayscale hover:grayscale-0 transition-all duration-500 opacity-60 hover:opacity-100">
                  <Image 
                    src={url} 
